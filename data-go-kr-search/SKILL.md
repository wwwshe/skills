...
```
data.go.kr 검색 API는 서비스키가 필요합니다. 공공데이터포털 오픈API는 서비스별 활용신청이 필요합니다.

1. https://www.data.go.kr 로그인
2. https://www.data.go.kr/data/15112888/openapi.do (공공데이터포털 검색 서비스)에서 활용신청 완료
3. 마이페이지 > 인증키 발급 > 일반 인증키(공공데이터포털 카탈로그 검색용)
4. 발급된 키를 아래 방법 중 하나로 설정해 주세요.
