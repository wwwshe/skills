# Claude Code Skills

Claude Code용 커스텀 스킬 모음.

## 설치

```bash
# 전체 설치
npx skills add wwwshe/skills

# 특정 스킬만
npx skills add https://github.com/wwwshe/skills/tree/main/<skill-name>
```

## 스킬 목록

| 스킬 | 설명 |
|------|------|
| [git-commit-gitflow-ko](./git-commit-gitflow-ko) | Git Flow 브랜치·한국어 커밋 메시지 규칙 |
| [data-go-kr-search](./data-go-kr-search) | 공공데이터포털 API/데이터셋 실시간 검색 |
| [anti-hallucination-research](./anti-hallucination-research) | 검색/조사형 AI의 환각 억제를 위한 검증 워크플로우 |
| [naver-shopping-search](./naver-shopping-search) | 네이버 쇼핑 공식 검색 API 실시간 상품 검색 |
| [naver-api-hub-search](./naver-api-hub-search) | NAVER API HUB 검색·검색어 트렌드 API |
| [flutter-clean-architecture](./flutter-clean-architecture) | Flutter Clean Architecture 스캐폴딩 |
| [kotlin-clean-architecture](./kotlin-clean-architecture) | Kotlin Clean Architecture 스캐폴딩 |
| [swift-mvvm](./swift-mvvm) | Swift MVVM 패턴 스캐폴딩 |
| [swiftui-mvvm](./swiftui-mvvm) | SwiftUI MVVM 상태/생명주기/비동기 패턴 가이드 |
| [swift-concurrency-mvvm](./swift-concurrency-mvvm) | UIKit MVVM에서 Swift Concurrency 경계/패턴 가이드 |
| [swift-combine-uikit-mvvm](./swift-combine-uikit-mvvm) | UIKit MVVM에서 Combine 바인딩 패턴 가이드 |
| [fastapi-backend](./fastapi-backend) | FastAPI + SQLAlchemy 2 + Pydantic v2 백엔드 |
